...
## Progress
  ...
  - [x] (2026-06-29 05:40Z) Stage 2 — v0.5.1 README clarity pass shipped. Bumped `package.json` to `0.5.1`, rewrote top of `README.md` and `README_ru.md` with positive framing (Anthropic quota = account-tied; `/context` = CLI-only), added `quota.png` + `limit.png` as two consecutive top screenshots, added "What's new in 0.5.1" sections, dropped the warning-style ⚠️ callout. Built `claude-limit-meter-0.5.1.vsix` (96.05 KB, both PNGs included). `install.ps1` reports `REGISTRY_UPDATED=0.5.1`. Commit `8de97c0` pushed to `origin/main`. GitHub Release [v0.5.1](https://github.com/cleverplant/claude-limit-meter/releases/tag/v0.5.1) created with VSIX attached. v0.5.0 release notes patched via REST API (PATCH 200) to add a "Superseded by v0.5.1" banner at the top.
